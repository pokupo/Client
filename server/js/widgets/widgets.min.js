...
var StatusPaymentWidget=function(){var j=this;j.widgetName="StatusPaymentWidget";j.version=1;j.minWidgetVersion=1;j.maxWidgetVersion=2;j.minTmplVersion=1;j.maxTmplVersion=2;j.InitWidget=g;var e={container:null,customContainer:null,tmpl:{path:null,id:null},status:null,backUrl:""};function g(){e.container=Config.Containers.statusPayment.widget;e.customContainer=Config.Containers.statusPayment.customClass;e.tmpl=Config.StatusPayment.tmpl;i();h();b()}function b(){var l={};if(Config.Base.sourceParameters=="string"){var k=JSCore.ParserInputParameters(/StatusPaymentWidget/);if(k.statusPayment){l=k.statusPayment}}if(Config.Base.sourceParameters=="object"&&typeof WParameters!=="undefined"&&WParameters.statusPayment){l=WParameters.statusPayment}if(JSSettings.dev){Logger.Console.VarDump(j.widgetName,"Input parameters",l)}if(!$.isEmptyObject(l)){if(l.tmpl){if(l.tmpl.path){e.tmpl.path=l.tmpl.path}if(l.tmpl.id){e.tmpl.id=l.tmpl.id}}if(l.animate){e.animate=l.animate}if(l.backUrl){e.backUrl=l.backUrl}}if(JSSettings.dev){Logger.Console.VarDump(j.widgetName,"Result settings",e)}j.settings=e}function a(){var k=$("#"+e.container).find(j.SelectCustomContent().join(", ")).clone();$("#"+e.container).empty().html(k)}function c(){a();$("#"+e.container).append($("script#"+j.GetTmplName()).html()).children().hide()}function h(){if(Routing.route=="status_payment"){var k=$.cookie(Config.Base.cookie.orderId);var m=$.cookie(Config.Base.cookie.userEmail);if(k){var n=Routing.GetParameter("name");var l=Routing.GetParameter("status");if(l){n=n+"/"+l}var o=n+"/?"+k+"&mailUser="+m;j.BaseLoad.Tmpl(e.tmpl,function(){j.BaseLoad.StatusPayment(o,function(p){j.BaseLoad.ShopInfo(function(q){d(p,q)})})})}else{j.WidgetLoader(true,e.container);alert("Нет параметров инициализации!")}}else{j.WidgetLoader(true,e.container)}}function i(){EventDispatcher.AddEventListener("w.change.route",function(k){h()});EventDispatcher.AddEventListener("StatusPaymentWidget.update",function(k){h()})}function d(k,m){var l=new StatusPaymentViewModel(k,m,e);c();f(l)}function f(k){if($("#"+e.container).length>0){try{ko.cleanNode($("#"+e.container)[0]);ko.applyBindings(k,$("#"+e.container)[0]);j.WidgetLoader(true,e.container);if(typeof AnimateStatusPayment=="function"){new AnimateStatusPayment()}if(e.animate){e.animate()}}catch(l){j.Exception("Ошибка шаблона ["+j.GetTmplName()+"]",l);if(e.tmpl.custom){delete e.tmpl.custom;j.BaseLoad.Tmpl(e.tmpl,function(){c();f(k)})}else{a();j.WidgetLoader(true,e.container)}}}else{j.Exception("Ошибка. Не найден контейнер ["+e.container+"]");j.WidgetLoader(true,e.container)}}};var StatusPaymentViewModel=function(d,e,c){var b=this;b.orderId=d.id_order;b.instruction=d.instruction;b.cssOrder="order_print_block";b.status=d.status;b.isPaid=ko.observable(false);if(b.status=="paid"){b.isPaid(true)}b.isWait=ko.observable(false);if(b.status=="wait_pay"){b.isWait(true)}b.isCancel=ko.observable(false);if(b.status=="cancel"){b.isCancel(true)}b.isBack=ko.observable(false);if(b.status=="back"){b.isBack(true)}b.outData=ko.observableArray();$.each(d.out_data,function(f){b.outData.push(new a(d.out_data[f]))});b.egoods=[];if(d.egoods){$.each(d.egoods,function(f){if(!d.egoods[f]["max_upload"]){d.egoods[f]["max_upload"]=false}if(!d.egoods[f]["expiration"]){d.egoods[f]["expiration"]=false}});b.egoods=d.egoods}b.Print=function(g){var f=window.open();f.document.write($("#"+g).html());f.print();f.close()};b.ClickPrintOrder=function(){b.Print(b.cssOrder)};b.ClickBack=function(){var f=e.site_shop;if(c.backUrl){f=c.backUrl}window.location.href=f};b.ClickRefresh=function(){EventDispatcher.DispatchEvent("StatusPaymentWidget.update")};b.showRefresh=false;if(Routing.GetParameter("status")=="success"&&b.status=="wait_pay"){b.showRefresh=true}function a(g){var f=this;f.label=ko.observable(g.label);f.value=ko.observable(g.value);f.help=ko.observable(g.help)}};var TestStatusPayment={Init:function(){if(typeof Widget=="function"){StatusPaymentWidget.prototype=new Widget();var a=new StatusPaymentWidget();a.Init(a)}else{setTimeout(function(){TestStatusPayment.Init()},100)}}};TestStatusPayment.Init();