...
var StatusPaymentWidget=function(){var a=this;a.widgetName="StatusPaymentWidget";a.version=1;a.minWidgetVersion=1;a.maxWidgetVersion=2;a.minTmplVersion=1;a.maxTmplVersion=2;a.settings={containerId:null,customContainer:null,tmpl:{path:null,id:null},inputParameters:{},status:null,style:null};a.InitWidget=function(){a.settings.containerId=Config.Containers.statusPayment.widget;a.settings.customContainer=Config.Containers.statusPayment.customClass;a.settings.tmpl=Config.StatusPayment.tmpl;a.settings.style=Config.StatusPayment.style;a.RegisterEvents();a.CheckRouteSearch();a.SetInputParameters();a.SetPosition()};a.SetInputParameters=function(){var c={};if(Config.Base.sourceParameters=="string"){var b=JSCore.ParserInputParameters(/StatusPaymentWidget/);if(b.statusPayment){c=b.statusPayment}}if(Config.Base.sourceParameters=="object"&&typeof WParameters!=="undefined"&&WParameters.statusPayment){c=WParameters.statusPayment}if(JSSettings.dev){Logger.Console.VarDump(a.widgetName,"Input parameters",c)}if(!$.isEmptyObject(c)){if(c.tmpl){if(c.tmpl.path){a.settings.tmpl.path=c.tmpl.path}if(c.tmpl.id){a.settings.tmpl.id=c.tmpl.id}}if(c.animate){a.settings.animate=c.animate}}a.settings.inputParameters=c;if(JSSettings.dev){Logger.Console.VarDump(a.widgetName,"Result settings",a.settings)}a.settings.inputParameters=c};a.InsertContainer={EmptyWidget:function(){var b=$("#"+a.settings.containerId).find(a.SelectCustomContent().join(", ")).clone();$("#"+a.settings.containerId).empty().html(b)},Content:function(){a.InsertContainer.EmptyWidget();$("#"+a.settings.containerId).append($("script#"+a.GetTmplName()).html()).children().hide()}};a.CheckRouteSearch=function(){if(Routing.route=="status_payment"){var b=$.cookie(Config.Base.cookie.orderId);var d=$.cookie(Config.Base.cookie.userEmail);if(b){var e=Routing.GetParameter("name");var c=Routing.GetParameter("status");if(c){e=e+"/"+c}var f=e+"/?"+b+"&mailUser="+d;a.BaseLoad.Tmpl(a.settings.tmpl,function(){a.BaseLoad.StatusPayment(f,function(g){a.BaseLoad.ShopInfo(function(h){a.Fill(g,h)})})})}else{a.WidgetLoader(true,a.settings.containerId);alert("Нет параметров инициализации!")}}else{a.WidgetLoader(true,a.settings.containerId)}};a.RegisterEvents=function(){EventDispatcher.AddEventListener("w.change.route",function(b){a.CheckRouteSearch()});EventDispatcher.AddEventListener("StatusPaymentWidget.update",function(b){a.CheckRouteSearch()})};a.Fill=function(b,d){var c=new StatusPaymentViewModel(b,d);a.InsertContainer.Content();a.Render(c)};a.Render=function(b){if($("#"+a.settings.containerId).length>0){try{ko.cleanNode($("#"+a.settings.containerId)[0]);ko.applyBindings(b,$("#"+a.settings.containerId)[0]);a.WidgetLoader(true,a.settings.containerId);if(typeof AnimateStatusPayment=="function"){new AnimateStatusPayment()}if(a.settings.animate){a.settings.animate()}}catch(c){a.Exception("Ошибка шаблона ["+a.GetTmplName()+"]",c);if(a.settings.tmpl.custom){delete a.settings.tmpl.custom;a.BaseLoad.Tmpl(a.settings.tmpl,function(){a.InsertContainer.Content();a.Render(b)})}else{a.InsertContainer.EmptyWidget();a.WidgetLoader(true,a.settings.containerId)}}}else{a.Exception("Ошибка. Не найден контейнер ["+a.settings.containerId+"]");a.WidgetLoader(true,a.settings.containerId)}};a.SetPosition=function(){if(a.settings.inputParameters.position=="absolute"){for(var b in a.settings.inputParameters){if(a.settings.style[b]){a.settings.style[b]=a.settings.inputParameters[b]}}$().ready(function(){if($("#"+a.settings.containerId).length>0){$("#"+a.settings.containerId).css(a.settings.style)}})}}};var StatusPaymentViewModel=function(b,c){var a=this;a.orderId=b.id_order;a.instruction=b.instruction;a.cssOrder="order_print_block";a.status=b.status;a.isPaid=ko.observable(false);if(a.status=="paid"){a.isPaid(true)}a.isWait=ko.observable(false);if(a.status=="wait_pay"){a.isWait(true)}a.isCancel=ko.observable(false);if(a.status=="cancel"){a.isCancel(true)}a.isBack=ko.observable(false);if(a.status=="back"){a.isBack(true)}a.outData=ko.observableArray();$.each(b.out_data,function(d){a.outData.push(new StatusPaymentParametersViewModel(b.out_data[d]))});a.egoods=[];if(b.egoods){$.each(b.egoods,function(d){if(!b.egoods[d]["max_upload"]){b.egoods[d]["max_upload"]=false}if(!b.egoods[d]["expiration"]){b.egoods[d]["expiration"]=false}});a.egoods=b.egoods}a.Print=function(e){var d=window.open();d.document.write($("#"+e).html());d.print();d.close()};a.ClickPrintOrder=function(){a.Print(a.cssOrder)};a.ClickBack=function(){window.location.href=c.site_shop};a.ClickRefresh=function(){EventDispatcher.DispatchEvent("StatusPaymentWidget.update")};a.showRefresh=false;if(Routing.GetParameter("status")=="success"&&a.status=="wait_pay"){a.showRefresh=true}};var StatusPaymentParametersViewModel=function(b){var a=this;a.label=ko.observable(b.label);a.value=ko.observable(b.value);a.help=ko.observable(b.help)};var TestStatusPayment={Init:function(){if(typeof Widget=="function"){StatusPaymentWidget.prototype=new Widget();var a=new StatusPaymentWidget();a.Init(a)}else{setTimeout(function(){TestStatusPayment.Init()},100)}}};TestStatusPayment.Init();